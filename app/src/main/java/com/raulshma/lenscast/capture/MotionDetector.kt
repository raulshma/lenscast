 ...
 * (caller dispatches). Disabled by default; sensitivity scales threshold;
 * detection narrows to the enabled [zones] when any exist. The grid verdict
 * fires when any considered tile breaches the threshold, so motion inside a
 * zone is not washed out by a still rest-of-frame.
 */
